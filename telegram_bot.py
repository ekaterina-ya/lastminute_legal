...
Этот бот проверяет рекламные креативы на соответствие ФЗ «О рекламе», опираясь на почти 2 тысячи решений ФАС, вынесенных за прошедшие 6 лет. Он работает по принципу Retrieval‑Augmented Generation (RAG): сначала ищет похожие кейсы, затем формирует ответ, обращаясь к нейросети Gemini 2.5 Pro (или Gemini 2.5 Flash, если Pro вдруг срабатывает с ошибками. Эта модель считается более слабой, так как не является «думающей», но и она, по нашей оценке, даёт неплохие заключения).
